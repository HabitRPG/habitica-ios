...
class SeasonalShopCollectionViewDataSource: ShopCollectionViewDataSource {
    ...
    override func loadCategories(_ categories: [ShopCategoryProtocol]) {
        ...
        for category in categories.sorted(by: { category1, category2 in
            return (category1.items.first?.currency == "gold" ? 1 : 0,
                    category1.identifier == "quests" ? 10000 : findReleaseYear(key: category1.items.first?.key ?? "")) > (category2.items.first?.currency == "gold" ? 1 : 0, category1.identifier == "quests" ? 10000 : findReleaseYear(key: category2.items.first?.key ?? ""))
        }) {

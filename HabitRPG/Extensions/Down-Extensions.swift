...
extension Down {
    ...
    func toHabiticaAttributedString(baseFont: UIFont) throws ->NSAttributedString {
        let string = try self.toAttributedString().mutableCopy() as! NSMutableAttributedString
        let fontSizeOffset = baseFont.pointSize - 12.0
        string.enumerateAttribute(NSFontAttributeName, in: NSRange(location: 0, length: string.length), options: NSAttributedString.EnumerationOptions.longestEffectiveRangeNotRequired, using: { (value, range, stop) in

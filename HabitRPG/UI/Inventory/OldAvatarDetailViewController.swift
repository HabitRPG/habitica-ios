...
class OldAvatarDetailViewController: BaseCollectionViewController, UICollectionViewDelegateFlowLayout {
    
    private var customizationDataSource: OldAvatarDetailViewDataSource?
    ...
    override func viewDidLoad() {
        ...
        if let type = customizationType {
            if type == "eyewear" || type == "headAccessory" || type == "back" || type == "animalTails" {
                gearDataSource = AvatarGearDetailViewDataSource(type: type, newCustomizationLayout: false)
                gearDataSource?.collectionView = collectionView
            } else {
                customizationDataSource = OldAvatarDetailViewDataSource(type: type, group: customizationGroup)
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let datasource = customizationDataSource, let customization = datasource.item(at: indexPath) {
            ...
            if datasource.owns(gear: gear) || gear is BlankGear {
                let key: String?

...
- (void)viewDidLoad {
    ...
    self.welcomeDescriptionString = NSLocalizedString(
        @"Welcome to Habitica, where advancing in the game will improve your real life! As you "
        @"accomplish real-world goals, you'll unlock equipment, pets, quests, and more.",
        nil);
    self.avatarDescriptionString = NSLocalizedString(
        @"First, you need an avatar in the game to represent you! The things you do in real life "
        @"will affect your avatar's health, experience level, and gold.",
        nil);
...
- (void)viewDidAppear:(BOOL)animated {
    if (self.currentStep == HRPGAvatarSetupStepsWelcome &&
        self.welcomeDescriptionLabel.text.length == 0) {
...
- (void)viewWillLayoutSubviews {
    ...
    if (self.currentStep == HRPGAvatarSetupStepsWelcome) {
        ...
        self.justinView.frame = CGRectMake((self.mainScrollView.frame.size.width - 84) / 2,
                                           self.mainScrollView.frame.size.height / 2 - 120, 84, 90);
        CGFloat descriptionHeight =
            [self.welcomeDescriptionString
                boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                ...
                .size.height +
            10;
        self.welcomeLabel.frame = CGRectMake(0, self.justinView.frame.origin.y - 50,
                                             self.mainScrollView.frame.size.width, 30);
        self.welcomeDescriptionLabel.frame =
            CGRectMake((self.mainScrollView.frame.size.width - labelWidth) / 2,
                       self.justinView.frame.origin.y + self.justinView.frame.size.height + 20,
                       labelWidth, descriptionHeight);
        self.welcomeButton.frame =
            CGRectMake(20, self.welcomeDescriptionLabel.frame.origin.y + descriptionHeight + 20,
                       self.mainScrollView.frame.size.width - 20, 60);
        height = self.welcomeButton.frame.origin.y + self.welcomeButton.frame.size.height;
    } else {
        CGFloat descriptionHeight =
            [self.avatarDescriptionString
                boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                             options:NSStringDrawingUsesLineFragmentOrigin
                          attributes:@{
                              NSFontAttributeName :
        ...
        self.descriptionlabel.frame =
            CGRectMake((self.mainScrollView.frame.size.width - labelWidth) / 2, 0, labelWidth,
                       descriptionHeight);

        CGFloat skinInstructionHeight =
            [self.skinInstructionlabel.text
                boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                             options:NSStringDrawingUsesLineFragmentOrigin
                          attributes:@{
                              NSFontAttributeName :
                ...
                .size.height +
            10;
        self.skinInstructionlabel.frame = CGRectMake(
            (self.mainScrollView.frame.size.width - labelWidth) / 2,
            self.descriptionlabel.frame.origin.y + self.descriptionlabel.frame.size.height + 30,
            labelWidth, skinInstructionHeight);
        self.skinSelectionView.frame =
            CGRectMake(20, self.skinInstructionlabel.frame.origin.y +
                               self.skinInstructionlabel.frame.size.height + 10,
                       self.mainScrollView.frame.size.width - 40, 250);
        [self.skinSelectionView layoutSubviews];
        [self.skinSelectionView sizeToFit];
        self.skinSeparatorView.frame = CGRectMake(0, self.skinInstructionlabel.frame.origin.y - 15,
                                                  self.mainScrollView.frame.size.width, 1);

        CGFloat shirtInstructionHeight =
            [self.shirtInstructionlabel.text
                boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                             options:NSStringDrawingUsesLineFragmentOrigin
                          attributes:@{
                              NSFontAttributeName :
                ...
                .size.height +
            10;
        self.shirtInstructionlabel.frame = CGRectMake(
            (self.mainScrollView.frame.size.width - labelWidth) / 2,
            self.skinSelectionView.frame.origin.y + self.skinSelectionView.frame.size.height + 30,
            labelWidth, shirtInstructionHeight);
        self.shirtSelectionView.frame =
            CGRectMake(20, self.shirtInstructionlabel.frame.origin.y +
                               self.shirtInstructionlabel.frame.size.height + 10,
                       self.mainScrollView.frame.size.width - 40, 250);
        [self.shirtSelectionView layoutSubviews];
        [self.shirtSelectionView sizeToFit];
        self.bodySizeView.frame = CGRectMake(
            (self.mainScrollView.frame.size.width / 2) - 50,
            self.shirtSelectionView.frame.origin.y + self.shirtSelectionView.frame.size.height + 10,
            100, 30);
        self.shirtSeparatorView.frame =
            CGRectMake(0, self.shirtInstructionlabel.frame.origin.y - 15,
                       self.mainScrollView.frame.size.width, 1);

        CGFloat hairStyleInstructionHeight =
            [self.hairStyleInstructionlabel.text
                boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                             options:NSStringDrawingUsesLineFragmentOrigin
                          attributes:@{
                              NSFontAttributeName :
                ...
                .size.height +
            10;
        self.hairStyleInstructionlabel.frame =
            CGRectMake((self.mainScrollView.frame.size.width - labelWidth) / 2,
                       self.bodySizeView.frame.origin.y + self.bodySizeView.frame.size.height + 30,
                       labelWidth, hairStyleInstructionHeight);
        self.hairBaseSelectionView.frame =
            CGRectMake(20, self.hairStyleInstructionlabel.frame.origin.y +
                               self.hairStyleInstructionlabel.frame.size.height + 10,
                       self.mainScrollView.frame.size.width - 40, 250);
        [self.hairBaseSelectionView layoutSubviews];
        [self.hairBaseSelectionView sizeToFit];
        self.hairBangsSelectionView.frame =
            CGRectMake(20, self.hairBaseSelectionView.frame.origin.y +
                               self.hairBaseSelectionView.frame.size.height + 20,
                       self.mainScrollView.frame.size.width - 40, 250);
        [self.hairBangsSelectionView layoutSubviews];
        [self.hairBangsSelectionView sizeToFit];
        self.hairStyleSeparatorView.frame =
            CGRectMake(0, self.hairStyleInstructionlabel.frame.origin.y - 15,
                       self.mainScrollView.frame.size.width, 1);

        CGFloat hairColorInstructionHeight =
            [self.hairColorInstructionlabel.text
                boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                             options:NSStringDrawingUsesLineFragmentOrigin
                          attributes:@{
                              NSFontAttributeName :
                ...
                .size.height +
            10;
        self.hairColorInstructionlabel.frame =
            CGRectMake((self.mainScrollView.frame.size.width - labelWidth) / 2,
                       self.hairBangsSelectionView.frame.origin.y +
                           self.hairBangsSelectionView.frame.size.height + 30,
                       labelWidth, hairColorInstructionHeight);
        self.hairColorSelectionView.frame =
            CGRectMake(20, self.hairColorInstructionlabel.frame.origin.y +
                               self.hairColorInstructionlabel.frame.size.height + 10,
                       self.mainScrollView.frame.size.width - 40, 250);
        [self.hairColorSelectionView layoutSubviews];
        [self.hairColorSelectionView sizeToFit];
        self.hairFlowerSelectionView.frame =
            CGRectMake(20, self.hairColorSelectionView.frame.origin.y +
                               self.hairColorSelectionView.frame.size.height + 10,
                       self.mainScrollView.frame.size.width - 40, 250);
        [self.hairFlowerSelectionView layoutSubviews];
        [self.hairFlowerSelectionView sizeToFit];
        self.hairColorSeparatorView.frame =
            CGRectMake(0, self.hairColorInstructionlabel.frame.origin.y - 15,
                       self.mainScrollView.frame.size.width, 1);

        height = self.hairFlowerSelectionView.frame.origin.y +
                 self.hairFlowerSelectionView.frame.size.height + 50;
    }
    [super viewWillLayoutSubviews];
...
- (void)setupWelcomeStep {
    ...
    [self.welcomeButton addTarget:self
...
- (void)setupCustomizationStep {
    ...
    self.skinSelectionView.items = [self
        getCustomizationsWithPredicate:[NSPredicate
    ...
    self.shirtInstructionlabel.text =
        NSLocalizedString(@"Choose your outfit! (You'll unlock more outfits soon.)", nil);
    ...
    self.shirtSelectionView.items = [self
        getCustomizationsWithPredicate:[NSPredicate
    ...
    [self.bodySizeView addTarget:self
                          action:@selector(userSizeChanged:)
    ...
    self.hairStyleInstructionlabel.font =
        [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
    ...
    self.hairBaseSelectionView.items = [self
        getCustomizationsWithPredicate:
    ...
    self.hairBangsSelectionView.items = [self
        getCustomizationsWithPredicate:
    ...
    self.hairColorInstructionlabel.font =
        [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
    ...
    self.hairColorSelectionView.items = [self
        getCustomizationsWithPredicate:
    ...
    self.hairFlowerSelectionView.items = [self
        getCustomizationsWithPredicate:
...
- (IBAction)nextStep:(id)sender {
    ...
    if (self.currentStep != HRPGAvatarSetupStepsAvatar) {
        ...
        HRPGAvatarSetupViewController *dest =
            [storyboard instantiateViewControllerWithIdentifier:@"AvatarSetupViewController"];
...
- (NSArray *)getCustomizationsWithPredicate:(NSPredicate *)predicate {
    ...
    [fetchRequest
        setSortDescriptors:@[ [NSSortDescriptor sortDescriptorWithKey:@"name" ascending:YES] ]];
    NSError *error;

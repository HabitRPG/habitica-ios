...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self.appstoreButton setTitle:NSLocalizedString(@"Open App Store page", nil) forState:UIControlStateNormal];
        [self.appstoreButton addTarget:self action:@selector(appstoreButtonPressed) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setMaintenanceData:(NSDictionary *) data {
    ...
    self.titleHeight = [[data objectForKey:@"title"] boundingRectWithSize:CGSizeMake(self.view.frame.size.width, MAXFLOAT)
...
context:nil]
    ...
    [manager
     requestImageWithURL:[NSURL URLWithString:[data objectForKey:@"imageUrl"]]
     options:0
     progress:nil
    ...
    NSAttributedString *descriptionText = [self renderMarkdown:[data objectForKey:@"description"]];
    
    self.descriptionTextView.attributedText = descriptionText;
    self.descriptionHeight = [self.descriptionTextView sizeThatFits:CGSizeMake(self.view.frame.size.width - 32, CGFLOAT_MAX)].height;

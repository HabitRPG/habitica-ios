...
- (void)loadObjectManager:(RKManagedObjectStore*)existingManagedObjectStore {
    ...
    responseDescriptor = [RKResponseDescriptor responseDescriptorWithMapping:entityMapping method:RKRequestMethodPOST pathPattern:@"/api/v2/groups/:id/leave" keyPath:nil statusCodes:RKStatusCodeIndexSetForClass(RKStatusCodeClassSuccessful)];
    [objectManager addResponseDescriptor:responseDescriptor];
...
- (void)setTimezoneOffset {
    ...
    if ([self.user.preferences.timezoneOffset integerValue] && offset != [self.user.preferences.timezoneOffset integerValue]) {
...
- (void)upDownTask:(Task *)task direction:(NSString *)withDirection onSuccess:(void (^)(NSArray *valuesArray))successBlock onError:(void (^)())errorBlock {
    ...
    [[RKObjectManager sharedManager] postObject:nil path:[NSString stringWithFormat:@"/api/v2/user/tasks/%@/%@", task.id, withDirection] parameters:nil success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        ...
        if ([self.user.level integerValue] < [taskResponse.level integerValue]) {
            ...
            self.user.experience = [NSNumber numberWithFloat:[self.user.experience floatValue] - [self.user.nextLevel floatValue]];
        }
        self.user.level = taskResponse.level ? taskResponse.level : self.user.level;

        NSNumber *expDiff = [NSNumber numberWithFloat:([taskResponse.experience floatValue] - [self.user.experience floatValue])];
        self.user.experience = taskResponse.experience;
        NSNumber *healthDiff = [NSNumber numberWithFloat:([taskResponse.health floatValue] - [self.user.health floatValue])];
        self.user.health = taskResponse.health ? taskResponse.health : self.user.health;
        NSNumber *magicDiff = [NSNumber numberWithFloat:([taskResponse.magic floatValue] - [self.user.magic floatValue])];
        self.user.magic = taskResponse.magic ? taskResponse.magic : self.user.magic;

        NSNumber *goldDiff = [NSNumber numberWithFloat:[taskResponse.gold floatValue] - [self.user.gold floatValue]];
        ...
        if (successBlock) {
            successBlock(@[healthDiff, expDiff, self.user.gold, self.user.health, self.user.experience, nextLevel, magicDiff]);
        }
        [self.networkIndicatorController endNetworking];
        return;
    }                                   failure:^(RKObjectRequestOperation *operation, NSError *error) {
...
- (void)buyObject:(MetaReward *)reward onSuccess:(void (^)())successBlock onError:(void (^)())errorBlock {
    ...
    [[RKObjectManager sharedManager] postObject:Nil path:[NSString stringWithFormat:@"/api/v2/user/inventory/buy/%@", reward.key] parameters:nil success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        ...
        NSNumber *goldDiff = [NSNumber numberWithFloat:[response.gold floatValue] - [self.user.gold floatValue]];
...
- (void)castSpell:(NSString *)spell withTargetType:(NSString *)targetType onTarget:(NSString *)target onSuccess:(void (^)())successBlock onError:(void (^)())errorBlock {
    ...
    [[RKObjectManager sharedManager] postObject:nil path:url parameters:nil success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        [self fetchUser:^(){
            NSError *executeError = nil;
            [[self getManagedObjectContext] saveToPersistentStore:&executeError];
            [self displaySpellNotification:(mana - [self.user.magic integerValue]) withHealthDiff:([self.user.health floatValue] - health) withGoldDiff:([self.user.gold floatValue] - gold)];
            if (successBlock) {
                successBlock();
            }
            [self.networkIndicatorController endNetworking];
...
- (void)inviteMembers:(NSArray *)members withInvitationType:(NSString *)invitationType toGroupWithID:(NSString*)group onSuccess:(void (^)())successBlock onError:(void (^)())errorBlock {
    ...
    [[RKObjectManager sharedManager] postObject:nil path:[NSString stringWithFormat:@"/api/v2/groups/%@/invite", group] parameters:@{invitationType: members, @"inviter": self.user.username} success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        NSError *executeError = nil;
        [[self getManagedObjectContext] saveToPersistentStore:&executeError];
        if (successBlock) {
            successBlock();
    ...
    [[RKObjectManager sharedManager] postObject:nil path:[NSString stringWithFormat:@"/api/v2/groups/%@/join", group] parameters:nil success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        ...
        if ([type isEqualToString:@"party"]) {
            ...
            if (![group.id isEqualToString:[defaults stringForKey:@"partyID"]]) {
                ...
                [defaults synchronize];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"partyChanged" object:group];
            }
        }
        group.isMember = @YES;
